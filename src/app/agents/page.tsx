...
const usdFormatter = new Intl.NumberFormat('tr-TR', { style: 'currency', currency: 'USD', minimumFractionDigits: 4, maximumFractionDigits: 4 });
...
export default function AgentsPage() {
  ...
  const usage = useMemo(() => runs.reduce((total, run) => ({
    input: total.input + (run.total_input_tokens || 0), output: total.output + (run.total_output_tokens || 0), cost: total.cost + Number(run.estimated_cost_usd || 0),
    unknownCostCount: total.unknownCostCount + (run.status === 'Hazır' && run.estimated_cost_usd == null ? 1 : 0),
  }), { input: 0, output: 0, cost: 0, unknownCostCount: 0 }), [runs]);
  ...
  return <Shell><div className="space-y-6">
    ...
    <div className="grid md:grid-cols-3 gap-3"><div className="bg-apex-card border border-apex-border rounded-xl p-4"><p className="text-[10px] text-apex-muted uppercase tracking-wider">Son 100 çalışma</p><p className="text-xl font-black text-white mt-1">{runs.length}</p></div><div className="bg-apex-card border border-apex-border rounded-xl p-4"><p className="text-[10px] text-apex-muted uppercase tracking-wider">Kaydedilen token</p><p className="text-xl font-black text-white mt-1">{tokenFormatter.format(usage.input + usage.output)}</p><p className="text-[10px] text-apex-muted mt-1">{tokenFormatter.format(usage.input)} giriş · {tokenFormatter.format(usage.output)} çıkış</p></div><div className="bg-apex-card border border-apex-border rounded-xl p-4"><p className="text-[10px] text-apex-muted uppercase tracking-wider">Tahmini API maliyeti</p><p className="text-xl font-black text-emerald-300 mt-1">{usage.unknownCostCount ? '—' : usdFormatter.format(usage.cost)}</p><p className="text-[10px] text-apex-muted mt-1">{usage.unknownCostCount ? `${usage.unknownCostCount} çalışma için oran tanımlanmadı.` : 'Fatura için OpenAI Costs ekranı esas alınır.'}</p></div></div>
    <div className="grid xl:grid-cols-[1.1fr_1.4fr] gap-5">
      ...
      <section className="bg-apex-card border border-apex-border rounded-2xl p-5 min-h-[620px]">
        ...
        {selected ? <div className="space-y-4"><div className="flex gap-2 flex-wrap"><span className="text-[10px] bg-apex-orange/15 text-apex-orange rounded px-2 py-1">{AGENT_ROLES.find((agent) => agent.id === selected.expert_role)?.label || selected.expert_role}</span><span className="text-[10px] bg-emerald-950 text-emerald-300 rounded px-2 py-1 flex items-center gap-1"><CheckCircle2 className="w-3 h-3"/>{selected.status}</span></div><div className="grid grid-cols-3 gap-2 text-[10px]"><div className="rounded-lg bg-apex-dark p-2 text-apex-muted">Model<strong className="block text-white mt-1 truncate">{selected.model || '—'}</strong></div><div className="rounded-lg bg-apex-dark p-2 text-apex-muted">Token<strong className="block text-white mt-1">{tokenFormatter.format((selected.total_input_tokens || 0) + (selected.total_output_tokens || 0))}</strong></div><div className="rounded-lg bg-apex-dark p-2 text-apex-muted">Tahmini maliyet<strong className="block text-emerald-300 mt-1">{selected.estimated_cost_usd == null ? '—' : usdFormatter.format(Number(selected.estimated_cost_usd))}</strong></div></div><p className="text-xs text-apex-muted border-l-2 border-apex-border pl-3">{selected.brief}</p><article className="whitespace-pre-wrap text-sm leading-7 text-neutral-200 bg-apex-dark border border-apex-border rounded-xl p-4">{selected.final_output || selected.error_message || 'Rapor hazırlanıyor…'}</article>{selected.expert_output && selected.expert_role !== 'genel-koordinator' && <details className="text-xs text-apex-muted"><summary className="cursor-pointer">Uzman taslağını görüntüle</summary><p className="whitespace-pre-wrap mt-3 bg-apex-dark rounded-lg p-3">{selected.expert_output}</p></details>}</div> : <div className="h-[480px] flex flex-col items-center justify-center text-center text-apex-muted"><Bot className="w-9 h-9 mb-3 text-apex-orange"/><p className="text-sm font-bold text-white">İlk görevi başlatın</p><p className="text-xs mt-2 max-w-sm">Uzman çıktısı ham hâlde bırakılmaz; Genel Koordinatör onu müşteriyle paylaşılabilir son rapora dönüştürür.</p></div>}</section>
    </div>
