...
export default function FinancePage() {
  ...
  const collected = (payment: Payment) => payment.status === 'Tamamlandı' ? Number(payment.amount) : Math.min(Number(payment.paid_amount) || 0, Number(payment.amount));
  const income = payments.reduce((sum, payment) => sum + collected(payment), 0);
  const expenseTotal = expenses.filter((expense) => expense.status === 'Ödendi').reduce((sum, expense) => sum + Number(expense.amount), 0);
  const receivables = payments.reduce((sum, payment) => sum + Math.max(0, Number(payment.amount) - collected(payment)), 0);
