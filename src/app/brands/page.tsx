...
export default function BrandsPage() {
  ...
  return <Shell><div className="space-y-6">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">{brands.map((brand) => <div key={brand.id} className="bg-apex-card border border-apex-border rounded-2xl p-5 hover:border-apex-orange/60"><h2 className="text-white font-bold">{brand.company_name}</h2><p className="text-xs text-apex-muted mt-1">{brand.sector || 'Sektör belirtilmedi'}</p><div className="mt-4 text-[11px] text-neutral-300 space-y-1"><p>{brand.contact_name || 'Yetkili kişi yok'} {brand.contact_phone ? `· ${brand.contact_phone}` : ''}</p><p className="truncate">{brand.website || brand.instagram || 'Kanal bilgisi yok'}</p>{brand.renewal_date && <p className="text-apex-orange">Yenileme: {brand.renewal_date}</p>}</div><div className="flex gap-2 mt-4"><Link href={`/brands/${brand.id}`} className="flex-1 text-center bg-apex-orange text-white rounded-lg py-2 text-xs font-bold">Müşteri Dosyası</Link><button onClick={() => { setEditing(brand); setShowForm(true); }} className="border border-apex-border rounded-lg px-3 text-xs text-apex-muted">Düzenle</button></div></div>)}{brands.length === 0 && <div className="col-span-full bg-apex-card border border-dashed border-apex-border rounded-2xl p-10 text-center text-xs text-apex-muted">Henüz marka kartı yok. Kazanılan müşteriyi seçerek ilk kartı oluşturun.</div>}</div>
  </div></Shell>;
}
