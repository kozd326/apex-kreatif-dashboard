...
function Rows({ rows, empty }: { rows: { title: string; sub: string; href?: string }[]; empty: string }) { return rows.length ? <>{rows.slice(0, 8).map((row, index) => row.href ? <Link key={`${row.title}-${index}`} href={row.href} className="block rounded-xl border border-apex-border bg-apex-dark/60 p-3 transition hover:border-apex-blue/60"><p className="text-xs font-bold text-white">{row.title}</p><p className="mt-1 text-[11px] text-apex-muted">{row.sub}</p><span className="mt-2 block text-[10px] font-bold text-apex-blue">Teklif belgesini aç →</span></Link> : <div key={`${row.title}-${index}`} className="rounded-xl border border-apex-border bg-apex-dark/60 p-3"><p className="text-xs font-bold text-white">{row.title}</p><p className="mt-1 text-[11px] text-apex-muted">{row.sub}</p></div>)}</> : <p className="py-6 text-center text-xs text-apex-muted">{empty}</p>; }
